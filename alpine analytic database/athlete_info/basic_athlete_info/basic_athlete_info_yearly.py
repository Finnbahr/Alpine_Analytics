...
def extract_athlete_data():
    ...
    then join them to extract fields needed for athlete aggregation.
    """
    with sqlite3.connect("fis_results.db") as conn:
        ...
        SELECT 
          ...
          fr.fis_points,
          rd.date,
          substr(rd.date, 1, 4) AS race_year,
          rd.discipline
        FROM fis_results fr
...
def process_athlete_data(df):
    """
    Process and aggregate the raw athlete data by fis_code, name, race_year, and discipline.
    Steps:
      1. Convert fis_points to numeric.
      2. For rows where rank starts with 'DNF', set fis_points to NaN.
      3. Convert bib to numeric and re-index bib values for each race (lowest bib becomes 1, etc.).
      4. Convert rank to numeric where possible.
      5. Create a DNF flag.
      6. Group the data and compute for each athlete:
         - Race count.
         - Aggregated statistics for fis_points, bib_index, and rank.
         - DNF rate and count.
         - **Average race_z_score.**
    """
    ...
    # For each race, compute a new bib index so that the lowest bib becomes 1, 2, etc.
    ...
    # Create a flag for DNF.
    df['dnf_flag'] = df['rank'].str.upper().str.startswith("DNF")
    
    # Group by athlete (fis_code, name), race_year, and discipline.
    grouped = df.groupby(['fis_code', 'name', 'race_year', 'discipline'])
...
def update_basic_athlete_info():
    ...
    process and aggregate, then store the enriched basic athlete info.
    """
    ...
    with sqlite3.connect(agg_db) as conn:
        df_agg.to_sql("basic_athlete_info_yearly", conn, if_exists="replace", index=False)
        conn.commit()
    
    print(f"'basic_athlete_info_yearly' table updated in '{agg_db}' at {datetime.now()}.")
